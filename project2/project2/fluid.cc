...
void copy_faces(float* vec, int nx, int skip, int idx) {
    ...
    int iskip, int jskip)
{
    ...
    double vol = dx * dy * dz;
    double sum = 0;
#pragma omp parallel for shared(u,v,w) reduction(+: sum)
    for (int i = 0; i < ni; ++i) {
        for (int j = 0; j < nj; ++j) {
            int offset = kstart + i * iskip + j * jskip;
            for (int k = 0; k < nk; ++k) {
...
void weightedSum3(float* uout, float w1, const float* u1, float w2,
    const float* u2, float w3, int ni, int nj, int nk, int kstart, int iskip, int jskip)
{
    const int kskip = 1;
